...
static void s_goto(ecs_state_t ns){ g_s = ns; g_t0 = millis(); }
...
bool end_credits_tick(void){
  ...
  switch(g_s){
    ...
    case ECS_TEAM: {
      ...
      /* center the 120x90 team photo */
      uint8_t x = (uint8_t)((128 - TEAM_W) / 2);
      uint8_t y = 24;
      gfx_blit565(x, y, TEAM_W, TEAM_H, TEAM);

      /* small Spartan badge over the bottom-right corner (scaled asset already 120x90) */
      uint8_t bx = (uint8_t)(128 - 1 - 32); /* 32x24 area */
      uint8_t by = (uint8_t)(128 - 1 - 24);
    ...
    case ECS_THANKS_MSU: {
      ...
      uint8_t y = 24;  /* same footprint as TEAM photo (120x90) */
