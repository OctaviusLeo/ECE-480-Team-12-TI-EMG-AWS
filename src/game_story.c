...
static void s_goto(story_state_t ns){ g_s = ns; g_t0 = millis(); }
...
void game_story_tick(void){
  ...
  switch(g_s){
    ...
    case STS_INTRO: {
      ...
      char line[40]; snprintf(line, sizeof(line), "%s  Target: %u Hz", c->enemy, (unsigned)c->enemy_hz);
      gfx_text2(6, 40, line, COL_YELLOW, 1);
      gfx_text2(6, 64, "Choose an item (A/B) with Hz", COL_DKGRAY, 1);
      choice_draw_hint(80);
      if (dt >= 2000u) s_goto(STS_CHOOSE);
    } break;
    ...
    case STS_CHOOSE: {
      ...
      // simple 1s settle before locking choice
      if (dt >= 1000u) {
    ...
    case STS_BATTLE: {
      ...
      if (dt >= FLEX_MS) s_goto(STS_RESULT);
    } break;
    ...
    case STS_RESULT: {
      ...
      gfx_text2(6, 76, (you >= foe) ? "VICTORY" : "DEFEAT", (you>=foe)?COL_GREEN:COL_RED, 2);

      if (dt >= 2000u) s_goto(STS_REWARD);
    } break;
    ...
    case STS_REWARD: {
      ...
      if (dt >= 1500u) s_goto(STS_NEXT);
    } break;

    case STS_NEXT: {
      uint8_t last = g_chapter;
      uint8_t next = (uint8_t)(g_chapter + 1u);
      if (next >= STORY_CHAPTERS) s_goto(STS_ENDING);
      else { g_chapter = next; s_goto(STS_INTRO); }
      (void)last;
    } break;
    ...
    case STS_ENDING: {
      ...
      gfx_text2(6, 70, "becoming the strongest in the lands", COL_WHITE, 1);
      gfx_text2(6, 90, "and saving the world!", COL_WHITE, 1);
      gfx_text2(6, 110, "or did you...", COL_GREEN, 1);
      if (dt >= 10000u) s_goto(STS_BRAND);
    } break;
  }
}
